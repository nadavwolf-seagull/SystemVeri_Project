...
class uart_rx_scoreboard extends uvm_scoreboard;
    ...
    function void write_mon(uart_rx_item observed);
        ...
        if ((observed.dut_parity_err  == exp_parity_err) &&
            (observed.dut_framing_err == exp_framing_err) &&
            ((exp_parity_err || exp_framing_err) ||
            (observed.data == sent.data))) begin
            matched++;
            `uvm_info(
                "SB",
                $sformatf(
                    {
                        "PASS: expected_data=0x%02h observed_data=0x%02h ",
                        "parity_err=%0b framing_err=%0b"
                    },
                    sent.data,
                    observed.data,
                    observed.dut_parity_err,
                    observed.dut_framing_err
                ),
                UVM_MEDIUM
            )
        end
